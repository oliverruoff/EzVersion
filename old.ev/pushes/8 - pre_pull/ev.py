...
def pull(push_id):
    ...
    for item in os.listdir(desired_pull_path):
        if item not in ignore_list:
            if os.path.isfile(item):
                shutil.copyfile(current_dir+'/'+item, current_dir+item)
            elif os.path.isdir(item):
                shutil.copytree(current_dir+'/'+item, current_dir+item)

    print('Desired push path:', desired_pull_path)
