# @auth: Oliver Ruoff
# for better use, copy this script to e.g. /usr/local/ev.py and create alias e.g. "alias ev='python /usr/local/ev.py"
...
#open(current_dir+'/.ev/push_map.txt', 'r').close()
	...
	while os.path.exists(push_path):
		push_tag = input('Tag '+push_tag+' already exists! Enter new (unique) tag.>>')
		push_path = pushes_path+push_tag+'/'
